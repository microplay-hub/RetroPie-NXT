...
rp_module_repo="git https://github.com/sikotik/Creativision.git master"
rp_module_licence="?"
...
function configure_creativision() {
    ...
    addEmulator 1 "${md_id}-crvision"  "crvision" "$md_inst/creatiVision -f -b %ROM%"
    addEmulator 0 "${md_id}-cslmode"  "crvision" "$md_inst/creatiVision -f -k -2 -b %ROM%"
    addEmulator 0 "${md_id}-saloramanager" "crvision" "$md_inst/creatiVision -f -k -3 -b %ROM%"
    addEmulator 0 "${md_id}-laser2001" "crvision" "$md_inst/creatiVision -f -3 -b %ROM%"
    addSystem "crvision"
